...
def prepare_dataset():
    ...
    studies = [results_path + str(study_ids[i]) +
               ".csv" for i in range(len(study_ids))]
    ...
    df['ms'] = df['ms'].map(lambda x: int(
        float(str(x).split(":")[2]) * 10000000))

    df['category'] = df['pattern'].map(
        lambda x: categorize(x)).astype("category")
    return df
